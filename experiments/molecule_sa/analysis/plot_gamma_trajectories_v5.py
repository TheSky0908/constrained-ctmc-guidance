"""Plot γ(=λ) and mean log p(y|x_t) trajectories for v5 adaptive_dual runs.

v5 = same C as v3 (C=−log 0.99 ≈ 0.01005), same ρ values (0.2, 0.5), but
sampling.steps=1024 instead of 128 (8× more steps). Each batch produces
trajectories of length 1024.
"""
import json
import os
...
RUNS = [
    ('C=−log 0.99, ρ=0.2, steps=1024  ⭐ best Viol@3.0 (6.89%)',
     'mdlm_dcbg_sa_n500_C0.01005_rho0.2_l00.0_lmax50.0_trainTau3.0_steps1024_traj.json',
     'tab:purple'),
    ('C=−log 0.99, ρ=0.5, steps=1024',
     'mdlm_dcbg_sa_n500_C0.01005_rho0.5_l00.0_lmax50.0_trainTau3.0_steps1024_traj.json',
     'tab:red'),
]
...
def main():
  ...
  for label, fn, color in RUNS:
    ...
    for k in [0, n_steps // 4, n_steps // 2, 3 * n_steps // 4, n_steps - 1]:
      print(f'  step {k:4d}: γ = {mean_lam[k]:.3f} ± {std_lam[k]:.3f}  '
  ...
  ax1.set_xlabel(
      'Sampling step  (0 = start, t=1, all masked  →  1023 = end, t≈0, clean)',
      fontsize=10)
  ax1.set_ylabel('γ = λ  (mean ± std over 500 trajectories)', fontsize=10)
  ax1.set_title(
      'Adaptive Dual γ(t) at sampling.steps=1024  (C = −log 0.99)',
      fontsize=11)
  ...
  fig.tight_layout()
  out_path = os.path.join(FIG_DIR, 'adaptive_dual_gamma_trajectory_v5.png')
