"""steps=256 version of plot_gamma_trajectories_samplefirst_timedep.py.

One 2-panel figure per C (C∈{−log0.8, −log0.99}), overlaying ρ∈{0.1, 0.2}.
Median line + 25–75% (IQR) band over the 500 trajectories.
Reads mdlm_dcbg_sa_adual_sf_td_C<C>_rho<RHO>_l02.0_lmax5.0_trainTau3.0_steps256_traj.json.
"""
import json
import os
...
C_VALUES = [('0.2231', '−log 0.8'), ('0.01005', '−log 0.99')]
RHOS = [('0.1', 'tab:blue'), ('0.2', 'tab:purple')]
L0, LMAX, TAU = '2.0', '5.0', '3.0'


def fn_for(c, rho):
  return (f'mdlm_dcbg_sa_adual_sf_td_C{c}_rho{rho}'
          f'_l0{L0}_lmax{LMAX}_trainTau{TAU}_steps256_traj.json')
...
def main():
  for c, c_label in C_VALUES:
    ...
    ax1.set_title(f'Adaptive Dual (sample_first, time-dep clf): '
                  f'γ(t) @ 256 steps  (C = {c_label})')
    ax1.set_xlabel('Sampling step  (0 = start, all masked → 255 = clean)')
    ...
    out = os.path.join(
        FIG_DIR, f'adual_samplefirst_timedep_traj_steps256_C{safe_c}.png')
