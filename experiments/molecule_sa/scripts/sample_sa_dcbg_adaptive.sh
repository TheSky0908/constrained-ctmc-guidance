#!/bin/bash
# Sample MDLM with D-CBG using the adaptive-dual γ schedule (Lagrangian dual
# variable λ updated via projected gradient ascent at each sampling step).
#
# Algorithm (per sample, per step):
#   λ ← clip( (λ - ρ · (log p(y|x_t) + C))_+ , 0, λ_max )
#   x_{t+1} ~ p(·|x_t) · p(y|·)^λ
#
# Usage:
#   bash sample_sa_dcbg_adaptive.sh [C] [RHO] [LAMBDA0] [LAMBDA_MAX] \
#                                    [N_BATCHES] [BATCH_SIZE] [TAU] [TAG]
#
# Defaults: C=0.2231 (=-log 0.8) RHO=0.5 LAMBDA0=0 LMAX=50 125 4 3.0 adual
...
# Format C without trailing zeros for filename. Include steps suffix only when
# non-default (128) so existing artifact filenames don't shift.
STEPS_SUFFIX=""
if [ "$SAMPLING_STEPS" != "128" ]; then STEPS_SUFFIX="_steps${SAMPLING_STEPS}"; fi
RUN_NAME=mdlm_dcbg_sa_${TAG}_C${C}_rho${RHO}_l0${LAMBDA0}_lmax${LMAX}_trainTau${TAU}${STEPS_SUFFIX}
SAMPLES_JSON=$OUT_DIR/${RUN_NAME}_samples.json
CSV=$OUT_DIR/${RUN_NAME}_results.csv

for p in "$DIFF_CKPT" "$CLF_CKPT"; do
  if [ ! -f "$p" ]; then
    echo "[sample_sa_dcbg_adaptive] MISSING CHECKPOINT: $p" >&2
    exit 1
  fi
done
...
echo "[$(date)] adaptive_dual: C=$C ρ=$RHO λ₀=$LAMBDA0 λmax=$LMAX train-τ=$TAU GPU=$CUDA_VISIBLE_DEVICES"
...
echo "[$(date)] adaptive_dual sampling done. samples=$SAMPLES_JSON csv=$CSV"
