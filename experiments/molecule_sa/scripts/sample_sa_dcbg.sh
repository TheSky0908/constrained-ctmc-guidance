...
# Backward-compatible naming: default (N=1000, steps=128) keeps the original
# filename; non-default N or steps add suffixes so runs don't collide.
N=$((NUM_BATCHES * BATCH_SIZE))
NTAG=""
if [ "$N" != "1000" ]; then NTAG="_n${N}"; fi
STEPS_SUFFIX=""
if [ "$SAMPLING_STEPS" != "128" ]; then STEPS_SUFFIX="_steps${SAMPLING_STEPS}"; fi
RUN_NAME=mdlm_dcbg_sa_gamma${GAMMA}${NTAG}_trainTau${TAU}${STEPS_SUFFIX}
