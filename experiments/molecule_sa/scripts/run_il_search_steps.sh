#!/bin/bash
# Reusable launcher for the inner-loop (Algm 2) hyperparameter search, STEPS parametrized.
# Setting: adual IL + time-dep classifier, N=500 (num_sample_batches x batch=4).
# Usage: run_il_search_steps.sh GPU C RHO L0 LMAX J N SEED TAG STEPS [NBATCH]
cd /local/scratch/zhiheng/guidance
GPU=$1; C=$2; RHO=$3; L0=$4; LMAX=$5; J=$6; NMC=$7; SEED=$8; TAG=$9; STEPS=${10}; NBATCH=${11:-125}
...
OUT=$ROOT/outputs/qm9/il_search128/${TAG}
mkdir -p $OUT
LOG=$ROOT/logs/il_search128_${TAG}.log
echo "[$TAG] GPU=$GPU C=$C rho=$RHO l0=$L0 lmax=$LMAX J=$J n=$NMC seed=$SEED steps=$STEPS nbatch=$NBATCH"
SECONDS=0
$PY -u sa_eval.py \
  ...
  sampling.steps=$STEPS sampling.num_sample_batches=$NBATCH \
