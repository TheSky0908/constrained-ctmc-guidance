#!/bin/bash
# Iterative parameter search driver for adaptive-dual sf+td on the NOVELTY task.
# Runs a list of (C,rho,l0,lmax,seed) combos SEQUENTIALLY on one GPU, times each,
# and appends a row to a shared master CSV so both GPUs' results land together.
# Idempotent: skips a combo whose per-run results CSV already exists.
#
# Usage:
#   CUDA_VISIBLE_DEVICES=2 bash search_adual_sftd.sh "C:RHO:L0:LMAX:SEED" ["..." ...]
#   (steps fixed at 64, N=500. target p(novel)=e^{-C}.)
#
# Dual update (sample_first / Algorithm 1):
#   lambda <- clip( (lambda - rho*(log p(novel|x_t) + C))_+ , 0, lambda_max )

set -uo pipefail
cd /local/scratch/zhiheng/guidance

# Default steps (per-combo overridable via optional 6th field "C:RHO:L0:LMAX:SEED:STEPS")
DEFAULT_STEPS=64
NUM_BATCHES=125
BATCH_SIZE=4
...
SEARCH_DIR=$OUT_DIR/adual_sftd_search
...
MASTER_HDR="C,rho,l0,lmax,seed,steps,seconds,valid,unique,valid_novel,viol,novel_rate,qed,run_name"

for p in "$DIFF_CKPT" "$CLF_CKPT"; do
  [ -f "$p" ] || { echo "[search_adual_sftd] MISSING CKPT: $p" >&2; exit 1; }
done

for combo in "$@"; do
  IFS=':' read -r C RHO L0 LMAX SEED STEPS <<< "$combo"
  SEED=${SEED:-1}
  SAMPLING_STEPS=${STEPS:-$DEFAULT_STEPS}
  MASTER=$OUT_DIR/adual_sftd_search_steps${SAMPLING_STEPS}.csv
  [ -f "$MASTER" ] || echo "$MASTER_HDR" > "$MASTER"
  RUN_NAME=adual_sftd_C${C}_rho${RHO}_l0${L0}_lmax${LMAX}_seed${SEED}_steps${SAMPLING_STEPS}
  ...
  echo "[$(date '+%F %T')] === C=$C rho=$RHO l0=$L0 lmax=$LMAX seed=$SEED steps=$SAMPLING_STEPS GPU=$CUDA_VISIBLE_DEVICES (target p=e^-C) ==="
  T0=$(date +%s)
  $PY -u $EVAL \
  ...
  if [ $RC -eq 0 ] && [ -f "$CSV" ]; then
    ...
    echo "$C,$RHO,$L0,$LMAX,$SEED,$SAMPLING_STEPS,$SEC,$VALID,$UNIQ,$VN,$VIOL,$NR,$QED,$RUN_NAME" >> "$MASTER"
    echo "[$(date '+%F %T')] [done] ${SEC}s valid=$VALID viol=$VIOL vn=$VN -> $MASTER"
  else
    echo "$C,$RHO,$L0,$LMAX,$SEED,$SAMPLING_STEPS,$SEC,FAIL,,,,,,$RUN_NAME" >> "$MASTER"
    echo "[$(date '+%F %T')] [FAIL] rc=$RC ${SEC}s — see $LOG"
  fi
done
echo "[$(date '+%F %T')] search batch done (GPU=$CUDA_VISIBLE_DEVICES)."
