"""Valid–Viol trade-off at steps=64 (N=500, seed=1) for three novelty D-CBG
methods, all using the ROLLOUT (eq.3, time-dependent) classifier:

  1. constant γ          — dcbg_gamma{0..10}_rollout_steps64_results.csv
  2. adaptive-dual sf+td  — adual_sftd_search_steps64.csv   (sample_first, Algm 1)
  3. adaptive-dual IL+td  — adual_iltd_search_steps64.csv   (inner-loop)
...
is drawn as a solid line per method. Saves figures/valid_viol_steps64.png.
"""
import csv
import os
...
def constant_gamma_points():
  """(viol%, valid, label) for γ=0..10, only valid>0 & viol defined."""
  pts = []
  for g in range(0, 11):
    fn = ('noguidance_rollout_steps64_results.csv' if g == 0
          else f'dcbg_gamma{g}_rollout_steps64_results.csv')
...
def search_points(fname):
  """(viol%, valid, run_name) for every row of a *_search_steps64.csv."""
...
def draw(methods, out, viol_max=None):
  """Draw the Pareto-frontier plot; if viol_max set, clip x to [−1, viol_max]
  and keep only frontier points within that range. Each method may carry an
  optional trim_top: drop that many highest-Viol points from its frontier."""
  ...
  title = ('Novelty D-CBG Valid–Viol trade-off (steps=64, N=500, seed=1)\n'
...
def main():
  methods = [
      ...
      ('adaptive-dual sf+td', search_points('adual_sftd_search_steps64.csv'),
       'tab:green', '^', 0),
      # drop the 2 highest-Viol points of the IL+td frontier
      ('adaptive-dual IL+td', search_points('adual_iltd_search_steps64.csv'),
       'tab:red', 's', 2),
  ]

  os.makedirs(FIGDIR, exist_ok=True)
  draw(methods, os.path.join(FIGDIR, 'valid_viol_steps64.png'))
  draw(methods, os.path.join(FIGDIR, 'valid_viol_steps64_viol20.png'),
       viol_max=20)

  # quick text summary of each method's low-Viol corner
