...
ARIS repo root: `/local/scratch/zhiheng/Auto-claude-code-research-in-sleep`
...
`ARIS_REPO=$(awk -F'\t' '$1=="repo_root"{print $2; exit}' "/local/scratch/zhiheng/guidance/.aris/installed-skills-codex.txt")`
Do not edit or delete symlinked skills in place; update upstream or rerun:
`bash /local/scratch/zhiheng/Auto-claude-code-research-in-sleep/tools/install_aris_codex.sh "/local/scratch/zhiheng/guidance" --reconcile`
For copied Codex installs, use:
`bash /local/scratch/zhiheng/Auto-claude-code-research-in-sleep/tools/smart_update_codex.sh --project "/local/scratch/zhiheng/guidance"`
<!-- ARIS-CODEX:END -->
